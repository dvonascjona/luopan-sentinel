...
变更内容：
- [新增] 小时报告逐个校验快照 `ts` 的北京时间日期，必须与报告日期一致。
- [新增] 快照缺少时间、时间非法或跨日时立即 `exit 1`，输出具体文件、时间和期望日期，并发飞书失败报警。
- [禁止] 时间校验失败后过滤坏文件继续生成报告，避免再次出现旧数据伪装成当天数据。

验收：正常快照执行 `hourly_report.js --dry-run` 必须退出 `0`；注入跨日快照后必须退出 `1` 且日志包含 `SNAPSHOT DATE MISMATCH`。
