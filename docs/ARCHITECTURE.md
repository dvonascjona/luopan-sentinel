...
└──────┬──────┘  └────────┬─────────┘  └──────┬───────┘
  ...
  data/YYYY-MM-DD_HH.json  screen_clean.json   route_b_clean.json
       ...
       ▼                  ▼
  ┌──────────────────────────────┐
  │  n8n webhook（live-data-clean）│
  │  Code 节点：合并 39+19+18 字段 │
  └──────────────┬───────────────┘
                 │
                 ▼
...
从"实时救火"到"长效复盘"的数字化管理：

| 层级 | 模块 | 核心功能 | 技术实现 |
|---|---|---|---|
| **采集层** | 罗盘/千川 | 每 5 分钟增量 + 每小时深度 | Playwright + Cookie 拦截 + Route B |
| **分析层** | 规则引擎 / AI | 实时异常诊断、下播复盘、主播评估 | Node.js 规则 + Claude Opus |
| **行动层** | 飞书生态 | 预警推送 + 数据看板 + 决策建议 | 飞书 Webhook / Bitable API |

### 功能模块

- **规则引擎**（已实现）：确定性强、零成本、毫秒级。流量暴跌/断层出单/效率低于均值 → 即时告警
- **AI 场次总结**（P2 计划）：下播触发，读全场时序 clean.json → 叙述式复盘报告
