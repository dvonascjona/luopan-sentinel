...
```bash
# 每小时归档 → n8n 清洗（:00 触发，30s 后 webhook）
0 0,1,8-23 * * * cd /opt/douyin-fetcher && (node live_capture_v3.js || (echo "[RETRY] 5min后重试" >> /tmp/luopan_cron.log && sleep 300 && node live_capture_v3.js)) >> /tmp/luopan_cron.log 2>&1 && sleep 30 && curl -s -X POST http://localhost:5678/webhook/live-data-clean >> /tmp/luopan_cron.log 2>&1
...
# 每小时 :30 Route B + extract + webhook 二次触发
30 8-23,0,1 * * * cd /opt/douyin-fetcher && flock -xn /tmp/route_b_pull.lock node route_b_pull.js >> /tmp/route_b_pull.log 2>&1 && node extract_screen_summary.js >> /tmp/route_b_pull.log 2>&1 && curl -s -X POST http://localhost:5678/webhook/live-data-clean >> /tmp/route_b_pull.log 2>&1
```
